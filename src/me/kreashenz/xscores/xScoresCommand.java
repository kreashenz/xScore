		...
		if (s instanceof Player) {
			...
			if (cmd.getName().equalsIgnoreCase("kdr")) {
				if (p.hasPermission("xscores.kdr")) {
					if (args.length == 0) {
						String kdr = String.valueOf(plugin.getKDR(p));
						p.sendMessage("§aYour KDR is : §9" + (kdr.length() >= 4 ? kdr.substring(0, 4) : kdr));
					} else {
						Player t = Bukkit.getPlayer(args[0]);
						if (t != null) {
							String kdr = String.valueOf(plugin.getKDR(t));
							p.sendMessage("§9" + t.getName() + "§a's KDR is §9" + (kdr.length() >= 4 ? kdr.substring(0, 4) : kdr));
